...
methods.createDatabase = (callback) => {
    ...
    sql = "CREATE TABLE IF NOT EXISTS SearchMetrics (\n" +
    ...
    conn.query(sql, function (error, results, fields) {
        ...
        console.log('DB: SearchMetrics table created with success')
        conn.end()
        return callback()
    });
};
...
methods.SearchMetricInsert = (searchInfo, callback) => {

    const conn = mysql.createConnection(dbInfo)
    var searchDate = new Date()

    var query = `INSERT INTO SearchMetrics SET 
                        user_id=${searchInfo.userId}, 
                        project_name="${searchInfo.projectName}", 
                        search_date=${conn.escape(searchDate)}`
